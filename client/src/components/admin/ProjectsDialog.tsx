...
const projectSchema = zod.object({
    ...
    impact_stats: zod.string().optional(), // Using string for simple impact input like "50k Trees"
...
export function ProjectsDialog({ open, onOpenChange, onSubmit, project, isSubmitting }: ProjectsDialogProps) {
    ...
    const handleFormSubmit = async (data: any) => {
        // Impact stats are handled as a simple string for now which the backend should treat as a translated field
        await onSubmit(data);
        form.reset();
        onOpenChange(false);
    };
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="space-y-4">
                                            ...
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                ...
                                                <SelectContent className="rounded-2xl border-slate-100 p-2 shadow-2xl">
                                                    <SelectItem value="Environment" className="rounded-xl font-bold py-3">Environment</SelectItem>
                                                    <SelectItem value="Health" className="rounded-xl font-bold py-3">Health</SelectItem>
                                                    <SelectItem value="Education" className="rounded-xl font-bold py-3">Education</SelectItem>
                                                    <SelectItem value="Humanitarian" className="rounded-xl font-bold py-3">Humanitarian</SelectItem>
                                                </SelectContent>
                                            </Select>
                                            <FormMessage className="text-xs font-bold text-rose-500 pl-1" />
                                        </FormItem>
                                            ...
                                            >
                                                {isUploading ? (
                                                    <div className="flex flex-col items-center gap-4">
                                                        ...
                                                        <span className="text-xs font-black uppercase tracking-widest text-primary animate-pulse italic">Uploading Banner...</span>
                                                    </div>
                                                    ...
                                                    <div className="flex flex-col items-center gap-4 text-slate-400 group-hover:text-primary transition-all duration-500 text-slate-900 leading-none">
                                                        ...
                                                        <span className="text-xs font-black uppercase tracking-[0.2em] leading-relaxed italic">Select project banner</span>
                                                    </div>
                                                )}
                                                <input
                                                    type="file"
